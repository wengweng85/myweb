/*
 * ! Amaze UI v2.7.2 ~ IE8 Fucker | by Amaze UI Team | (c) 2016 AllMobilize,
 * Inc. | Licensed under MIT | 2016-08-17T16:17:24+0800
 */
/*
...
(function(root, factory) {
	...
	var fnToStr = Function.prototype.toString, constructorRegex = /^\s*class /, isES6ClassFn = function isES6ClassFn(
			value) {
		try {
	...
	}, fnClass = '[object Function]', genClass = '[object GeneratorFunction]', isCallable = function isCallable(
			value) {
		if (!value) {
			return false;
		}
	...
	var regexExec = RegExp.prototype.exec, tryRegexExec = function tryRegexExec(
			value) {
		try {
	...
	var strValue = String.prototype.valueOf, tryStringObject = function tryStringObject(
			value) {
		try {
			strValue.call(value);
			return true;
	...
	/*
	 * replaceable with https://npmjs.com/package/es-abstract
	 * /helpers/isPrimitive
	 */
		...
		bind : function bind(that) { // .length is 1
			...
			var binder = function() {
				...
				if (this instanceof bound) {
					...
					var result = apply.call(target, this, array_concat.call(
			...
			// extensions,
			// for ex.) all use of eval or Function costructor throws an
			// exception.
			// However in all of these environments Function.prototype.bind
			// exists
	...
	defineProperties(ArrayPrototype, {
		every : function every(callbackfn/* , thisArg */) {
			...
			for (var i = 0; i < length; i++) {
				if (i in self
						&& !(typeof T === 'undefined' ? callbackfn(self[i], i,
								object) : callbackfn
								.call(T, self[i], i, object))) {
					return false;
				}
			}
			return true;
	...
	if (ArrayPrototype.reduceRight) {
		reduceRightCoercesToObject = typeof ArrayPrototype.reduceRight.call(
				'es5', function(_, __, ___, list) {
					return list;
				}) === 'object';
	}
	defineProperties(ArrayPrototype, {
	...
	defineProperties(ArrayPrototype, {
				splice : function splice(start, deleteCount) {
					...
					var actualStart = relativeStart < 0 ? max(
							(len + relativeStart), 0) : min(relativeStart, len);
					var actualDeleteCount = min(max(ES.ToInteger(deleteCount),
									0), len - actualStart);
		...
		defineProperties(ArrayPrototype, {
					join : function join(separator) {
						var sep = typeof separator === 'undefined'
								? ','
								: separator;
						return originalJoin.call(isString(this) ? strSplit(
										this, '') : this, sep);
					}
				}, hasStringJoinBug);
	}
	...
	var pushIsNotGeneric = (function() {
		...
		return result !== 1 || obj.length !== 1
				|| typeof obj[0] !== 'undefined' || !owns(obj, 0);
	}());
	defineProperties(ArrayPrototype, {
	...
	var pushUndefinedIsWeird = (function() {
		...
		return result !== 1 || arr.length !== 1
				|| typeof arr[0] !== 'undefined' || !owns(arr, 0);
	}());
	defineProperties(ArrayPrototype, {
				push : pushShim
	...
	// can be replaced with require('is-arguments') if we ever use a build
	// process instead
	...
	var isLegacyArguments = function isArguments(value) {
		return value !== null && typeof value === 'object'
				...
				&& !isArray(value) && isCallable(value.callee);
	};
	...
	var keysHasArgumentsLengthBug = $Object.keys && (function() {
		...
		return arguments.length !== 1 || argKeys.length !== 1
				|| argKeys[0] !== 1;
	}(1));
	...
	if (timeZoneOffset < -720) {
		...
		hasToStringFormatBug = !(/^Thu Dec 10 2015 \d\d:\d\d:\d\d GMT[-\+]\d\d\d\d(?: |$)/)
				.test(aPositiveTestDate.toString());
	} else {
	...
	var originalGetUTCMilliseconds = call
			.bind(Date.prototype.getUTCMilliseconds);
	...
	defineProperties(Date.prototype, {
		toISOString : function toISOString() {
			...
			year = ((year < 0 ? '-' : (year > 9999 ? '+' : '')) + strSlice(
					'00000' + Math.abs(year), (0 <= year && year <= 9999)
							? -4
							: -6));
		...
		Date = (function(NativeDate) {
			...
			var DateShim = function Date(Y, M, D, h, m, s, ms) {
				...
				if (this instanceof NativeDate) {
					var seconds = s;
					var millis = ms;
					if (hasSafariSignedIntBug && length >= 7
							&& ms > maxSafeUnsigned32Bit) {
						// work around a Safari 8/9 bug where it treats the
						// seconds as signed
			...
			var parseShim = function parse(string) {
				...
				if (match) {
					...
					isLocalTime = Boolean(match[4] && !match[8]), signOffset = match[9] === '-'
							? 1
							: -1, hourOffset = $Number(match[10] || 0), minuteOffset = $Number(match[11]
							|| 0), result;
					var hasMinutesOrSecondsOrMilliseconds = minute > 0
	...
	var hasToFixedBugs = NumberPrototype.toFixed
			&& ((0.00008).toFixed(3) !== '0.000' || (0.9).toFixed(0) !== '1'
					|| (1.255).toFixed(2) !== '1.25' || (1000000000000000128)
	...
	if ('ab'.split(/(?:ab)*/).length !== 2
			|| '.'.split(/(.?)(.?)/).length !== 4
			|| 'tesst'.split(/(s)*/)[1] === 't'
			|| 'test'.split(/(?:)/, -1).length !== 4 || ''.split(/.?/).length
			|| '.'.split(/()()/).length > 1) {
		(function() {
			...
			StringPrototype.split = function(separator, limit) {
				 ...
				 * maxSafe32BitInt If 0, Infinity, or NaN: 0 If positive number:
	...
	defineProperties(StringPrototype, {
				lastIndexOf : function lastIndexOf(searchString) {
					...
					var numPos = arguments.length > 1
							? $Number(arguments[1])
							: NaN;
					var pos = isActualNaN(numPos) ? Infinity : ES
							.ToInteger(numPos);
					...
					while (k > 0) {
						...
						var index = strIndexOf(
								strSlice(S, k, start + searchLen), searchStr);
						if (index !== -1) {
							return k + index;
						}
					}
	...
	if (parseInt(ws + '08') !== 8 || parseInt(ws + '0x16') !== 22) {
		...
		parseInt = (function(origParseInt) {
			...
			return function parseInt(str, radix) {
				var string = trim(str);
				var defaultedRadix = $Number(radix)
						|| (hexRegex.test(string) ? 16 : 10);
	...
	if (String(new RangeError('test')) !== 'RangeError: test') {
		...
		// can't use defineProperties here because of toString enumeration issue
		// in IE <= 8
	...
	if (!Object.getPrototypeOf) {
		...
		Object.getPrototypeOf = function getPrototypeOf(object) {
			...
			if (proto || proto === null) {
				...
				// `Object.create(null)`
				// (shammed or native) or `{ __proto__: null}`. Also returns
				// null for
				// cross-realm objects on browsers that lack `__proto__` support
				// (like
	...
	if (!Object.defineProperty || definePropertyFallback) {
		...
		Object.defineProperty = function defineProperty(object, property,
			...
			if ('value' in descriptor) {
				 ...
				 * features; allow false but not true ('writable' in descriptor &&
				 * !descriptor.writable) || ('enumerable' in descriptor &&
				 * !descriptor.enumerable) || ('configurable' in descriptor &&
				 * !descriptor.configurable) )) throw new RangeError( 'This
...
(function(global) {
	...
	var methods = ('assert,clear,count,debug,dir,dirxml,error,exception,group,'
			+ 'groupCollapsed,groupEnd,info,log,markTimeline,profile,profiles,profileEnd,' + 'show,table,time,timeEnd,timeline,timelineEnd,timeStamp,trace,warn')
			.split(',');
...
!window.addEventListener
		&& (function(WindowPrototype, DocumentPrototype, ElementPrototype,
				addEventListener, removeEventListener, dispatchEvent, registry) {
			WindowPrototype[addEventListener] = DocumentPrototype[addEventListener] = ElementPrototype[addEventListener] = function(
					type, listener) {
				var target = this;
			...
			WindowPrototype[removeEventListener] = DocumentPrototype[removeEventListener] = ElementPrototype[removeEventListener] = function(
				...
				for (var index = 0, register; register = registry[index]; ++index) {
					if (register[0] == this && register[1] == type
							&& register[2] == listener) {
...
/**
 * Module: rem - v1.3.4 Description: A polyfill to parse CSS links and rewrite
 * pixel equivalents into head for non supporting browsers Date Built:
 * 2014-07-02 Copyright (c) 2014 | Chuck Carpenter
 * <chuck.carpenter@me.com>,Lucas Serven <lserven@gmail.com>;
 * 
...
(function(window, undefined) {
	...
	storeCSS = function(response, link) {
		...
		if (CSSLinks.length === links.length) {
			...
			if ((links = importLinks.slice(0)).length > 0) { // after
																// finishing all
																// current
																// links, set
																// links equal
	...
	matchCSS = function(sheetCSS, link) { // collect all of the rules from the
											// xhr response texts and match them
											// to a pattern
		var clean = removeMediaQueries(sheetCSS).replace(/\/\*[\s\S]*?\*\//g,
				''), // remove MediaQueries and comments
		pattern = /[\w\d\s\-\/\\\[\]:,.'"*()<>+~%#^$_=|@]+\{[\w\d\s\-\/\\%#:!;,.'"*()]+\d*\.?\d+rem[\w\d\s\-\/\\%#:!;,.'"*()]*\}/g, // find
																																	// selectors
																																	// that
																																	// use
																																	// rem
																																	...
																																	// their
																																	// rules
		current = clean.match(pattern), remPattern = /\d*\.?\d+rem/g, remCurrent = clean
				.match(remPattern), sheetPathPattern = /(.*\/)/, sheetPath = sheetPathPattern
				.exec(link)[0], // relative path to css file specified in
								// @import
		importPattern = /@import (?:url\()?['"]?([^'\)"]*)['"]?\)?[^;]*/gm, // matches
																			// all
																			// @import
																			// variations
																			// outlined
		...
		while ((importStatement = importPattern.exec(sheetCSS)) !== null) {
			if (importStatement[1].indexOf("/") === 0) { // check if the
															// value of
															// importStatement[1]
															// is a root
															// relative path, in
		...
		if (current !== null && current.length !== 0) {
			found = found.concat(current); // save all of the blocks of rules
											// with rem in a property
			foundProps = foundProps.concat(remCurrent); // save all of the
														// properties with rem
		}
	},

	buildCSS = function() { // first build each individual rule from elements in
							// the found array and then add it to the string of
							// rules.
		var pattern = /[\w\d\s\-\/\\%#:,.'"*()]+\d*\.?\d+rem[\w\d\s\-\/\\%#:!,.'"*()]*[;}]/g; // find
																								// properties
																								// with
																								// rem
																								// values
		...
		for (var i = 0; i < found.length; i++) {
			rules = rules + found[i].substr(0, found[i].indexOf("{") + 1); // save
																			// the
																			// selector
																			// portion
																			// of
			...
			for (var j = 0; j < current.length; j++) { // build a new set of
														// with only the
														// selector and
	...
	parseCSS = function() { // replace each set of parentheses with evaluated
							// content
		for (var i = 0; i < foundProps.length; i++) {
			css[i] = Math.round(parseFloat(foundProps[i].substr(0,
					foundProps[i].length - 3)
					* fontSize))
					+ 'px';
		}
	...
	loadCSS = function() { // replace and load the new rules
		for (var i = 0; i < css.length; i++) { // only run this loop as many
												// times as css has entries
			if (css[i]) {
				rules = rules.replace(foundProps[i], css[i]); // replace old
																// rules with
																// our processed
																// rules
			}
		...
		document.getElementsByTagName('head')[0].appendChild(remcss); // create
																		// the
																		// new
																		// element
		if (remcss.styleSheet) {
			remcss.styleSheet.cssText = rules; // IE8 will not support
												// innerHTML on read-only
	...
	xhr = function(url, callback) { // create new XMLHttpRequest object and run
									// it
		try {
			// try to create a request object
			// arranging the two conditions this way is for IE7/8's benefit
			// so that it works with any combination of ActiveX or Native XHR
			// settings,
			// as long as one or the other is enabled; but if both are enabled
			// it prefers ActiveX, which means it still works with local files
			// (Native XHR in IE7/8 is blocked and throws "access is denied",
			// but ActiveX is permitted if the user allows it [default is to
			// prompt])
	...
	removeMediaQueries = function(css) {
		...
		if (!window.matchMedia && !window.msMatchMedia) {
			// If the browser doesn't support media queries, we find all @media
			// declarations in the CSS and remove them.
			// Note: Since @rules can't be nested in the CSS spec, we're safe to
			// just check for the closest following "}}" to the "@media".
	...
	if (!cssremunit()) { // this checks if the rem value is supported
		var rules = '', // initialize the rules variable in this scope so it can
						// be used later
		links = isStyleSheet(), // initialize the array holding the sheets urls
								// for use later
		importLinks = [], // initialize the array holding the import sheet
							// urls for use later
		found = [], // initialize the array holding the found rules for use
					// later
		foundProps = [], // initialize the array holding the found properties
							// for use later
		preCSS = [], // initialize array that holds css before being parsed
		CSSLinks = [], // initialize array holding css links returned from xhr
		css = [], // initialize the array holding the parsed rules for use
					// later
		fontSize = '';

		// Notice: rem is a "root em" that means that in case when html element
		// size was changed by css
		// or style we should not change document.documentElement.fontSize to
		// 1em - only body size should be changed
		// to 1em for calculation

		fontSize = (function() {
			var doc = document, docElement = doc.documentElement, body = doc.body
					|| doc.createElement('body'), isFakeBody = !doc.body, div = doc
					.createElement('div'), currentSize = body.style.fontSize, size;
...
!function(a) {
	...
	a.matchMedia = a.matchMedia || function(a) {
		var b, c = a.documentElement, d = c.firstElementChild || c.firstChild, e = a
				.createElement("body"), f = a.createElement("div");
		return f.id = "mq-test-1", f.style.cssText = "position:absolute;top:-100em", e.style.background = "none", e
				.appendChild(f), function(a) {
	...
	if (c.ajax = f, c.queue = d, c.unsupportedmq = g, c.regex = {
		...
		var h, i, j, k = a.document, l = k.documentElement, m = [], n = [], o = [], p = {}, q = 30, r = k
				.getElementsByTagName("head")[0]
				|| l, s = k.getElementsByTagName("base")[0], t = r
				.getElementsByTagName("link"), u = function() {
			var a, b = k.createElement("div"), c = k.body, d = l.style.fontSize, e = c
					&& c.style.fontSize, f = !1;
			return b.style.cssText = "position:absolute;font-size:1em;width:1em", c
					|| (c = f = k.createElement("body"), c.style.background = "none"), l.style.fontSize = "100%", c.style.fontSize = "100%", c
					.appendChild(b), f && l.insertBefore(c, l.firstChild), a = b.offsetWidth, f
					? l.removeChild(c)
					: c.removeChild(b), l.style.fontSize = d, e
					&& (c.style.fontSize = e), a = j = parseFloat(a)
		}, v = function(b) {
			var c = "clientWidth", d = l[c], e = "CSS1Compat" === k.compatMode
					&& d || k.body[c] || d, f = {}, g = t[t.length - 1], p = (new Date)
					.getTime();
			if (b && h && q > p - h)
			...
			for (var s in m)
				if (m.hasOwnProperty(s)) {
					...
					x
							&& (x = parseFloat(x)
									* (x.indexOf(B) > -1 ? j || u() : 1)), y
							&& (y = parseFloat(y)
									* (y.indexOf(B) > -1 ? j || u() : 1)), w.hasquery
							&& (z && A || !(z || e >= x) || !(A || y >= e))
							|| (f[w.media] || (f[w.media] = []), f[w.media]
									.push(n[w.rules]))
				}
			for (var C in o)
			...
			for (var j = 0; f > j; j++) {
				...
				i ? (k = d, n.push(h(a))) : (k = e[j].match(c.regex.findStyles)
						&& RegExp.$1, n.push(RegExp.$2 && h(RegExp.$2))), o = k
						.split(","), p = o.length;
				for (var q = 0; p > q; q++)
					l = o[q], g(l) || m.push({
								media : l.split("(")[0].match(c.regex.only)
										&& RegExp.$2 || "all",
								rules : n.length - 1,
			...
			if (d.length) {
				var b = d.shift();
				f(b.href, function(c) {
							w(c, b.href, b.media), p[b.href] = !0, a
									.setTimeout(function() {
												x()
											}, 0)
						})
			...
			for (var b = 0; b < t.length; b++) {
				var c = t[b], e = c.href, f = c.media, g = c.rel
						&& "stylesheet" === c.rel.toLowerCase();
				e
						&& g
						&& !p[e]
						&& (c.styleSheet && c.styleSheet.rawCssText
								...
								: (!/^([a-zA-Z:]*\/\/)/.test(e) && !s || e
										.replace(RegExp.$1, "").split("/")[0] === a.location.host)
										&& ("//" === e.substring(0, 2)
												&& (e = a.location.protocol + e), d
												.push({
															href : e,
															media : f
														})))
		...
		y(), c.update = y, c.getEmValue = u, a.addEventListener ? a
				.addEventListener("resize", b, !1) : a.attachEvent
